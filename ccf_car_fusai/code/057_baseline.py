...
path = ''
train_sales_data = pd.read_csv(path+'../ccf_car/train_sales_data.csv')#, engine='python')
...
data = pd.merge(train_sales_data, train_search_data, 'left', on=['adcode', 'model', 'regYear', 'regMonth'])
...
df_lgb.to_csv(path + "../rst/057_baseline_df_lgb.csv", index=False) 
